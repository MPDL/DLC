...
public class ImageHelper {
	...
	static {
		try {
			THUMBNAILS_DIR = PropertyReader
					.getProperty("image-upload.thumbnailsDir") + "/";
			WEB_DIR = PropertyReader.getProperty("image-upload.webDir") + "/";
			ORIGINAL_DIR = PropertyReader
					.getProperty("image-upload.originalDir") + "/";
		} catch (Exception e) {
	...
	{
		try {
			String mainUrl = PropertyReader
					.getProperty("image-upload.url.download");
			String typeUrl = "";
			switch (type) {
			case THUMBNAIL: {
	...
	public static File scaleImage(File inputFile, String fileName, Type type)
			throws Exception {
		...
		switch (type) {
		case THUMBNAIL: {
			widthString = PropertyReader
					.getProperty("image.thumbnail.maxWidth");
			heightString = PropertyReader
					.getProperty("image.thumbnail.maxHeight");
			break;
		}
		case WEB: {
		...
		case ORIGINAL: {
			...
			heightString = PropertyReader
					.getProperty("image.original.maxHeight");
			break;
		}
		}
	...
	public static BufferedImage scaleImage(BufferedImage image, int width,
		...
		if (widthRatio > heightRatio) {
			...
			float newHeight = (float) originalHeight / (float) originalWidth
					* width;

			resizeImage = resizeImage(image, width, (int) newHeight);
		} else {

			float newWidth = (float) originalWidth / (float) originalHeight
					* height;
	...
	private static BufferedImage resizeImage(BufferedImage image, int width,
		...
		// BufferedImage scaledImage = Scalr.resize(image, Scalr.Method.QUALITY,
		// width, height);
		...
		BufferedImage scaledImage = new BufferedImage(width, height,
				image.getType());
		scaledImage.getGraphics().drawImage(resized, 0, 0, null);
		 ...
		 * resizedImage.createGraphics(); g.drawImage(image, 0, 0, width,
	...
	public static File mergeImages(File inputFile, File footer) {
		try {
			...
			BufferedImage finalImg = new BufferedImage(scanWidth, scanHeight
					+ footerHeight, scanBI.getType());
	...
	public static File tiffToJpeg(File tiffFile, String name) {
		...
		try {
			...
			logger.info("tmpFile Name = " + tmpFile.getName() + " | Path = "
					+ tmpFile.getAbsolutePath());
			logger.info("Name = " + tiffFile.getName() + " | Path = "
					+ tiffFile.getAbsolutePath() + " | Size = "
					+ tiffFile.length());
			...
			ImageEncoder imageEncoder = ImageCodec.createImageEncoder("JPEG",
					fos, jParam);
			...
			logger.error(
					"cann not convert tiff to jpeg. Error: " + e.getMessage()
							+ " | Size = " + tmpFile.length(), e);
		}
		return tmpFile;
	}
	...
	public static File tiffToPng(File tiffFile, String name) {
		...
		try {
			...
			// ImageEncoder imageEncoder = ImageCodec.createImageEncoder(name,
			// dst, param)("PNG", fos);
			// imageEncoder.encode(ri);
			...
			logger.error("cann not convert tiff to png. Error: "
					+ e.getMessage() + " | Size = " + tmpFile.length());
		}
	...
	public static File jpegToPng(File jpegFile, String name) {
		...
		try {
			...
			logger.error("cann not convert tiff to jpeg. Error: "
					+ e.getMessage() + " | Size = " + tmpFile.length());
		}
	...
	public static File pngToJpeg(File pngFile, String name) {
		...
		try {
			...
			/*
			 * Problem with transparent png Files tmpFile =
			 * File.createTempFile(name, "jpg.tmp"); SeekableStream s = new
			 * FileSeekableStream(pngFile); PNGDecodeParam param = new
			 * PNGDecodeParam(); ImageDecoder dec =
			 * ImageCodec.createImageDecoder("png", s, param); RenderedImage ri
			 * = dec.decodeAsRenderedImage(0);
			 * 
			 * FileOutputStream fos = new FileOutputStream(tmpFile);
			 * JPEGEncodeParam jParam = new JPEGEncodeParam();
			 * jParam.setQuality(1.0f); ImageEncoder imageEncoder =
			 * ImageCodec.createImageEncoder("JPEG", fos, jParam);
			 * imageEncoder.encode(ri); fos.flush(); fos.close();
			 */

		} catch (Exception e) {
			logger.error("cann not convert png to jpeg. Error: "
					+ e.getMessage());
		}
	...
	public static void convertTiffToJpeg(String inputDic, String outputDic)
			throws IOException {
	...
	public static void main(String[] arg) throws Exception {
		/*
		 * File f = new
		 * File("C:\\Users\\haarlae1\\Pictures\\TestBilder\\1.jpg");
		 * BufferedImage bufferedImage = ImageIO.read(f); String destDir="test";
		 * String imgType="jpg";
		 * 
		 * String dir = PropertyReader.getProperty("image-upload.destDir") +
		 * "/original/" + destDir; ImageHelper.scaleAndStoreImage(f,
		 * f.getName(), bufferedImage, f.getName(), imgType, Type.ORIGINAL);
		 * 
		 * dir = PropertyReader.getProperty("image-upload.destDir") + "/web/" +
		 * destDir; ImageHelper.scaleAndStoreImage(dir,
		 * Integer.parseInt(PropertyReader
		 * .getProperty("image-upload.resolution.web")), bufferedImage,
		 * f.getName(), imgType, Type.WEB);
		 * 
		 * dir = PropertyReader.getProperty("image-upload.destDir") +
		 * "/thumbnails/" + destDir; ImageHelper.scaleAndStoreImage(dir,
		 * Integer.
		 * parseInt(PropertyReader.getProperty("image-upload.resolution.thumbnail"
		 * )), bufferedImage,f.getName(), imgType, Type.THUMBNAIL);
		 */

		// String inputDic =
		// "C://Doku//dlc//TEST_MPDL//ROM//batch//images//Va6400-1990a";
		// String outputDic =
		// "C://Doku//dlc//TEST_MPDL//ROM//batch//jpegimages//Va6400-1990a";
		// convertTiffToJpeg(inputDic, outputDic);

		File tif = new File("/home/frank/data/digitization_lifecycle/test/berlin/bw-images/tif/Zusammenfassung.tif");
		File jpg = tiffToJpeg(tif, tif.getName().concat(".jpg"));
		File scaled = scaleImage(jpg, tif.getName().concat(".web"), Type.WEB);

	}

	/*
	 * display image metadata public static void displayMetadata(File file)
	 * throws Exception {
	 * 
	 * ImageInputStream iis = ImageIO.createImageInputStream(file);
	 * Iterator<ImageReader> readers = ImageIO.getImageReaders(iis); if
	 * (readers.hasNext()) { // pick the first available ImageReader ImageReader
	 * reader = readers.next(); // attach source to the reader
	 * reader.setInput(iis, true); // read metadata of first image IIOMetadata
	 * metadata = reader.getImageMetadata(0); String[] names =
	 * metadata.getMetadataFormatNames(); int length = names.length; for (int i
	 * = 0; i < length; i++) { System.out.println( "Format name: " + names[ i ]
	 * ); displayMetadata(metadata.getAsTree(names[i]), 0); } }
	 * 
	 * } public static void indent(int level) { for (int i = 0; i < level; i++)
	 * System.out.print("    "); } static void displayMetadata(Node node, int
	 * level) { // print open tag of element indent(level); System.out.print("<"
	 * + node.getNodeName()); NamedNodeMap map = node.getAttributes(); if (map
	 * != null) {
	 * 
	 * // print attribute values int length = map.getLength(); for (int i = 0; i
	 * < length; i++) { Node attr = map.item(i); System.out.print(" " +
	 * attr.getNodeName() + "=\"" + attr.getNodeValue() + "\""); } }
	 * 
	 * Node child = node.getFirstChild(); if (child == null) { // no children,
	 * so close element and return System.out.println("/>"); return; }
	 * 
	 * // children, so close current tag System.out.println(">"); while (child
	 * != null) { // print children recursively displayMetadata(child, level +
	 * 1); child = child.getNextSibling(); }
	 * 
	 * // print close tag of element indent(level); System.out.println("</" +
	 * node.getNodeName() + ">"); }
	 */
