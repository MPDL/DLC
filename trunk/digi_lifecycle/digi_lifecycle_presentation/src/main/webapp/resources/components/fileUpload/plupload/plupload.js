...
(function() {
	...
	g.Uploader = function(q) {
		...
		function n() {
			...
			if (t.size === b) {
				t.percent = s.length > 0 ? Math.ceil(t.uploaded / s.length
						* 100) : 0
			} else {
				t.bytesPerSec = Math.ceil(t.loaded
						/ ((+new Date() - p || 1) / 1000));
		...
		g.extend(this, {
			...
			init : function() {
				...
				z.bind("UploadProgress", function(B, C) {
					C.percent = C.size > 0 ? Math.ceil(C.loaded / C.size * 100)
							: 100;
					n()
				});
