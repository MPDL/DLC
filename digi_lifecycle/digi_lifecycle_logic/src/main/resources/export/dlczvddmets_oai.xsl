	...
	<OAI-PMH xmlns="http://www.openarchives.org/OAI/2.0/"
         ...
         xsi:schemaLocation="http://www.openarchives.org/OAI/2.0/
  		...
  		<xsl:element name="request">
  			...
  			<xsl:attribute name="identifier">oai:escidoc.org:<xsl:value-of select="$itemId"/></xsl:attribute>
  			<xsl:attribute name="metadataPrefix"><xsl:value-of select="'zvdd'"/></xsl:attribute>
  		</xsl:element>
		<GetRecord>
			<record>
				 <oai:header>
					<oai:identifier>oai:escidoc.org:<xsl:value-of select="$itemId"/> </oai:identifier> 
