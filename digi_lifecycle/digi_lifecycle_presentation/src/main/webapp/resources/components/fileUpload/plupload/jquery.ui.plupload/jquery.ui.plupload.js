					...
					"ui.plupload",
					{
						...
						_create : function() {
							...
							this.filelist = g(".plupload_filelist_content",
									this.container).attr("id", k + "_filelist");
							...
							if (i.options.max_file_count) {
								j.bind("FilesAdded", function(l, n) {
									var m = n.length, o = [];
									if (m > i.options.max_file_count) {
										o = n.splice(i.options.max_file_count);
										l.trigger("Error", {
							...
							j.bind("UploadProgress", function(l, m) {
								g("#" + m.id + " .plupload_file_status",
										i.element).html(m.percent + "%");
								i._handleFileStatus(m);
						...
						_handleState : function() {
							...
							if (j.state === c.STARTED) {
								...
								g([]).add(i.stop_button).add(
										".plupload_started").removeClass(
										"plupload_hidden");
								g(".plupload_upload_status", i.element).text(
										b("Uploaded %d/%d files").replace(
												"%d/%d",
												j.total.uploaded + "/"
														+ j.files.length));
								g(".plupload_header_content", i.element)
						...
						_updateTotalProgress : function() {
							...
							this.progressbar.progressbar("value",
									i.total.percent);
							g(".plupload_total_status", this.element).html(
									i.total.percent + "%");
							g(".plupload_upload_status", this.element).text(
									b("Uploaded %d/%d files").replace(
											"%d/%d",
											i.total.uploaded + "/"
													+ i.files.length))
						...
						_updateFileList : function() {
							var j = this, n = this.uploader, l = this.filelist, k = 0, o, m = this.id
									+ "_", i;
							...
							g
									.each(
											...
											function(q, p) {
												...
												j._trigger("updatelist", null,
														l)
											});
							g(".plupload_total_file_size", j.element).html(
									c.formatSize(n.total.size));
							if (n.total.queued === 0) {
								g(".ui-button-text", j.browse_button).text(
										b("Add Files"))
							} else {
								g(".ui-button-text", j.browse_button).text(
							...
							if (!n.files.length && n.features.dragdrop
									&& n.settings.dragdrop) {
						...
						_enableRenaming : function() {
							var i = this;
							g(".plupload_file_name span", this.filelist)
									.live(
											"click",
											function(o) {
									...
									.sortable(
											{
												...
												stop : function(q, p) {
													...
													for (
															l = 0,
															o = i.uploader.files.length; l < o; l++) {
														if (l === m) {
															k = j
														} else {
															if (l === j) {
