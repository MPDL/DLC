...
<xsl:stylesheet version="2.0"
	...
	xmlns="http://www.loc.gov/mods/v3" exclude-result-prefixes="mab">
	...
	<xsl:variable name="ALTERNATIVE_TITLE"
		select="'alternative'" />
	<xsl:variable name="ABBREVIATED_TITLE"
		select="'abbreviated'" />
	<xsl:variable name="TRANSLATED_TITLE"
		select="'translated'" />
	...
	<xsl:template match="/">
		<xsl:element name="mods:mods">
				...
				<xsl:element name="mods:part">
					...
					<xsl:if test="//mab:feld[@nr='090']">
						<xsl:attribute name="order">
							<xsl:value-of select="//mab:feld[@nr='090']" />
						</xsl:attribute>
					</xsl:if>
			...
			<xsl:for-each select="key('fields', '334')">
				<xsl:call-template
					name="mab334ToPhysicalDescription" />
			</xsl:for-each>
			...
			<xsl:for-each select="key('fields', '403')">
				...
				<!-- mab425ToOriginInfo necessary if there is no 410 -->
			</xsl:for-each>
			<xsl:for-each select="key('fields', '410')">
				<xsl:call-template name="mab410ToOriginInfo" />
				<!-- mab410ToOriginInfo includes 410, 410x, 412, 412x checks for 403 
					and 425 -->
			</xsl:for-each>

			<xsl:for-each select="key('fields', '415')">
				<xsl:call-template name="mab415ToOriginInfo" />
				<!-- mab415ToOriginInfo includes 415, 417 checks for 403 and 425 -->
			</xsl:for-each>
			...
			<xsl:for-each select="key('fields', '433')">
				<xsl:call-template
					name="mab433ToPhysicalDescription" />
			</xsl:for-each>
			...
			<xsl:for-each select="key('fields', '750')">
				<xsl:call-template name="mab750ToAbstract" />
			</xsl:for-each>
		...
		<xsl:element name="mods:recordInfo">
			<xsl:element name="mods:recordIdentifier">
				<xsl:attribute name="source">
					<xsl:value-of select="concat($IDPREFIX, @nr)" />
				</xsl:attribute>
				<xsl:value-of select="." />
			</xsl:element>
		</xsl:element>
		...
		<xsl:element name="mods:name">
			<xsl:if test="@ind=' ' or @ind='a'">
				...
				<xsl:if test="@nr='100'">
					...
					<xsl:if test="key('fields', '102')">
						<xsl:attribute name="valueURI">
							<xsl:value-of select="key('fields', '102')" />
						</xsl:attribute>
					</xsl:if>
				</xsl:if>
				<xsl:if test="@nr='104'">
					...
					<xsl:if test="key('fields', '106')">
						<xsl:attribute name="valueURI">
							<xsl:value-of select="key('fields', '106')" />
						</xsl:attribute>
					</xsl:if>
				</xsl:if>
				<xsl:if test="@nr='108'">
					...
					<xsl:if test="key('fields', '110')">
						<xsl:attribute name="valueURI">
							<xsl:value-of select="key('fields', '110')" />
						</xsl:attribute>
					</xsl:if>
				</xsl:if>
			</xsl:if>
			<xsl:if test="not(@ind=' ' or @ind='a' or @ind='f')">
				<xsl:attribute name="ID">
					<xsl:value-of
					select="concat($IDPREFIX, concat(@nr, @ind))" />
				</xsl:attribute>
				<xsl:if test="@nr='100'">
					<xsl:attribute name="displayLabel">
			...
			<xsl:element name="mods:namePart">
				<xsl:choose>
					<xsl:when test="mab:uf[@code='p']">
						<xsl:value-of select="mab:uf[@code='p']" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="." />
					</xsl:otherwise>
	...
	<xsl:template name="mab200ToName">
		<xsl:element name="mods:name">
			<xsl:if test="@ind=' ' or @ind='a'">
				...
				<xsl:if test="@nr='200'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body1'" />
					</xsl:attribute>
				</xsl:if>
				<xsl:if test="@nr='204'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body2'" />
					</xsl:attribute>
				</xsl:if>
				<xsl:if test="@nr='208'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body3'" />
					</xsl:attribute>
				</xsl:if>
			</xsl:if>
			<xsl:if test="not(@ind=' ' or @ind='a')">
				...
				<xsl:if test="@nr='200'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body4'" />
					</xsl:attribute>
				</xsl:if>
				<xsl:if test="@nr='204'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body5'" />
					</xsl:attribute>
				</xsl:if>
				<xsl:if test="@nr='208'">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="'body6'" />
					</xsl:attribute>
				</xsl:if>
			</xsl:if>
			<xsl:attribute name="type">
	...
	<xsl:template name="mab310ToAlternativeTitle">
		<xsl:element name="mods:titleInfo">
			...
			<xsl:attribute name="type">
				<xsl:value-of select="$ALTERNATIVE_TITLE" />
			</xsl:attribute>
			<xsl:if test="@nr='341'">
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'parallelTitle'" />
				</xsl:attribute>
			</xsl:if>
			<xsl:element name="mods:title">
				<xsl:value-of select="." />
	...
	<xsl:template name="mab331ToMainTitle">
		<xsl:element name="mods:titleInfo">
			...
			<xsl:if test="@ind='a'">
				...
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'NEB follow the title'" />
				</xsl:attribute>
				<xsl:element name="mods:title">
					<xsl:value-of select="." />
				</xsl:element>
			...
			<xsl:if test="@ind='b'">
				...
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'NEB with the title'" />
				</xsl:attribute>
				<xsl:element name="mods:title">
					<xsl:value-of select="." />
				</xsl:element>
	...
	<xsl:template name="mab359ToNote">
		<xsl:element name="mods:note">
			...
			<xsl:attribute name="type">
				<xsl:value-of select="'statement of responsibility'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab365ToNote">
		<xsl:element name="mods:note">
			...
			<xsl:attribute name="type">
				<xsl:value-of
				select="'remainder of title for whole item'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab369ToNote">
		<xsl:element name="mods:note">
			...
			<xsl:attribute name="type">
				<xsl:value-of select="'reponsibility for whole item'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab403ToOriginInfo">
		<xsl:choose>
			<xsl:when test="key('fields', '410')"></xsl:when>
			<xsl:otherwise>
		...
		<xsl:element name="mods:originInfo">
			...
			<xsl:if test="key('fields', '425')">
				<xsl:element name="mods:dateIssued">
					...
					<xsl:value-of
						select="if (key('fields', '425')[@ind=' ']) then key('fields', '425')[@ind=' '] else key('fields', '425')[@ind='a']" />
				</xsl:element>
			</xsl:if>
	...
	<xsl:template name="mab415ToOriginInfo">
		<xsl:element name="mods:originInfo">
			<xsl:if test="@ind=' '">
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'publisher2'" />
				</xsl:attribute>
			</xsl:if>
			<xsl:if test="@ind='a'">
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'printer2'" />
				</xsl:attribute>
			</xsl:if>
			<xsl:element name="mods:place">
				<xsl:element name="mods:placeTerm">
	...
	<xsl:template name="mab429ToNote">
		<xsl:element name="mods:note">
			...
			<xsl:attribute name="type">
				<xsl:value-of select="'subject completeness'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab432ToNote">
		<xsl:element name="mods:note">
			...
			<xsl:attribute name="type">
				<xsl:value-of select="'summary of volumes'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab611ToRelatedItem">
		<xsl:element name="mods:originInfo">
			<xsl:if test="@ind=' '">
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'publisher3'" />
				</xsl:attribute>
				<xsl:element name="mods:place">
					<xsl:element name="mods:placeTerm">
						<xsl:attribute name="type">
	...
	<xsl:template name="mab647ToRelatedItem">
		<xsl:element name="mods:note">
			<xsl:attribute name="type">
				<xsl:value-of select="'digital master'" />
			</xsl:attribute>
			<xsl:value-of select="." />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="mab655ToRelatedItem">
		<xsl:element name="mods:location">
			<xsl:element name="mods:url">
				<xsl:attribute name="displayLabel">
					<xsl:value-of select="'electronic resource'" />
				</xsl:attribute>
	...
	<xsl:template name="mab750ToAbstract">
